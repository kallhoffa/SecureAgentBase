...
2. **Push to `main`** → `.github/workflows/firebase-deploy-staging.yml` deploys hosting + Firestore rules to staging. This run skips `E2E_FULL` (wizard full-mode), but **Build CLI + Run CLI E2E Tests now run on every push to `main`** (the CLI e2e is a push gate, not just a release gate — a broken CLI fails the run instead of hiding until release day).
